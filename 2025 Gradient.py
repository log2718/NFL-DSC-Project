...
import numpy as np
import joblib
from Gradient_Model import load_model, clean_numeric_column, predict_from_snapshot

MODEL_PATH = "Models/final_winpct_model.joblib"
...
if "Wins" in snapshot.columns:
    ...
    snapshot["current_WL_pct"] = snapshot["W - L %"].astype(float)
else:
...
snapshot[["Team", "pred_final_win_pct", "pred_wins", "pred_losses"]].to_csv(OUTPUT_PATH, index=False)
